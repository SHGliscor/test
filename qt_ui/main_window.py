...
class MainWindow(QMainWindow):
    ...
    def _wild_movement_page(self):
        ...
        self.movement_mode.addItem("Wiggle — Left / Right only","wiggle_lr")
        self.movement_mode.addItem("Wiggle — Up / Down only","wiggle_ud")
    ...
    def _oak_challenge_page(self):
        ...
        self.oak_shiny_mode=QComboBox()
        self.oak_shiny_mode.addItem("Non-shiny only","non_shiny")
        self.oak_shiny_mode.addItem("Shiny only","shiny")
        self.oak_shiny_mode.addItem("Any","any")
        form.addRow("Encounter type",self.oak_shiny_mode)
    ...
    def _support_page(self):
        w=QWidget(); v=QVBoxLayout(w); c,l=self.card("Testing / Support")
        self.log_label=QLabel("No support errors this run."); self.log_label.setWordWrap(True)
        self.support_label=QLabel("No support ZIP created this run."); self.support_label.setWordWrap(True)
        ...
        exportb.setToolTip(
            ...
            "Discord bot token is never included."
        )
    ...
    def export_support_zip(self):
        ...
        try:
            self.worker.request_export_support("manual_export")
            self.log_label.setText("Exporting support ZIP…")
        except Exception as exc:
            self.log_label.setText(f"Export failed to start: {exc}")

    def _load_settings_ui(self):
        self.movement_mode.setCurrentIndex(max(0,self.movement_mode.findData(self.settings.get("wild_movement_mode","wiggle_lr"))))
    ...
    def _finish_session(self,msg=None):
        was_hunting=self.hunting; elapsed=self._elapsed(); self.store.stop_session(elapsed); self.session_elapsed_frozen=elapsed; self.session_started_mono=None; self.session_started_epoch=None; self.hunting=False; self.last_status=msg or "Session stopped"; self.start_btn.setEnabled(True); self.stop_btn.setEnabled(False); self.release_btn.setEnabled(self.connected); self.group_combo.setEnabled(True); self.target.setEnabled(True); self.gc_count.setEnabled(True); self.movement_mode.setEnabled(True); self.spin_hold.setEnabled(True); self.spin_settle.setEnabled(True); self.auto_capture.setEnabled(True); self.shiny_auto_capture.setEnabled(True); self.capture_ball_slot.setEnabled(True); self.capture_max_throws.setEnabled(True); self.egg_release_nonshiny.setEnabled(True); self.egg_max_eggs.setEnabled(True); self.egg_hatch_hold.setEnabled(True); self.egg_hatch_cycles.setEnabled(True); self.oak_stage.setEnabled(True); self.oak_shiny_mode.setEnabled(True); self.oak_targets.setEnabled(True); self.oak_blocked.setEnabled(True); self.rng_shiny.setEnabled(True); self.rng_nature.setEnabled(True); self.rng_max.setEnabled(True); self.rng_lead.setEnabled(True); [b.setEnabled(True) for b in self.rng_iv_boxes]; self.hunt_state.setText(self.last_status); self._hunt_changed(); self._refresh_stats(); self._push_discord_snapshot()
